...
app.get('/api/proveedores', (req: Request, res: Response) => {
...
app.get('/api/materiales', (req: Request, res: Response) => {
...
app.post('/api/materiales', (req: Request, res: Response) => {
...
app.put('/api/materiales/:id', (req: Request, res: Response) => {
  const { id } = req.params;
  const { nombre, cantidad, unidad_medida, stock_minimo } = req.body;
...
app.delete('/api/materiales/:id', (req: Request, res: Response) => {
  const { id } = req.params;
...
app.get('/api/compras', (req: Request, res: Response) => {
  const query = `
...
app.get('/api/obras', (req: Request, res: Response) => {
  const query = `
...
app.post('/api/obras', (req: Request, res: Response) => {
...
app.put('/api/obras/:id', (req: Request, res: Response) => {
  const { id } = req.params;
  const { id_cliente, nombre, direccion, presupuesto, fecha_inicio, estado } = req.body;
...
app.delete('/api/obras/:id', (req: Request, res: Response) => {
  const { id } = req.params;
...
app.get('/api/consumos', (req: Request, res: Response) => {
  const query = `
...
app.post('/api/consumos', (req: Request, res: Response) => {
  ...
  db.query(qVerificarStock, [id_material], (errCheck, resultsCheck) => {
    if (errCheck) return res.status(500).json({ error: 'Error al verificar stock' });
    
    const filas = resultsCheck as any[];
    if (filas.length === 0) return res.status(404).json({ error: 'Material no encontrado' });
    ...
    db.beginTransaction((errTx) => {
      if (errTx) return res.status(500).json({ error: 'Error al iniciar la transacción' });
      ...
      db.query(qInsertConsumo, [id_obra, id_material, cantidad], (errInsert) => {
        ...
        db.query(qRestarStock, [cantidad, id_material], (errUpdate) => {
          ...
          // Confirmar cambios
          db.commit((errCommit) => {
            if (errCommit) return db.rollback(() => res.status(500).json({ error: 'Error al confirmar' }));
...
app.get('/api/empleados', (req: Request, res: Response) => {
  const query = `
...
app.put('/api/empleados/:id', (req: Request, res: Response) => {
  const { id } = req.params;
...
// El servidor Express escucha en el puerto de la API (5000)
